...
dyn_create_markers = {
    ...
    _marker9 setMarkerText "1./Team YANKEE";
...
dyn_main_setup = {
    ...
    // _locations = nearestLocations [_center, ["NameVillage", "NameCity"], worldSize * 0.3];
    // _startLoc = selectRandom _locations;
    // _startPos = getPos _startLoc;
    // private _campaignDir = ((getPos _startLoc) getDir _center) - 180;
    ...
    [dyn_locations, _playerStart, _campaignDir] spawn {
        ...
        for "_i" from 0 to (count _locations) - 1 do {
            ...
            if (_midDefenses) then {
                ...
                _defenseType = selectRandom ["line", "point", "mobileTank", "roadem"];
            ...
            if (_outerDefenses) then {
                ...
                _defenseType = selectRandom ["line", "point", "mobileTank", "roadem", "empty"];
            ...
            if (_i < ((count _locations) - 1)) then {
                if ((random 1) > 0.5) then {
                    [_endTrg, getPos (_locations#_i) , getPos (_locations#(_i + 1)), 5, 3, 4, false, dyn_standart_combat_vehicles, 1000] spawn dyn_spawn_counter_attack;
