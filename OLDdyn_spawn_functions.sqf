...
dyn_spawn_parked_vehicle = {
    params ["_pos", "_area", ["_vicTypes", ["cwr3_o_bmp1", "cwr3_o_btr80", "cwr3_o_bmp2", "cwr3_o_t55"]], ["_roadDir", 0]];
    private ["_vPos"];
...
dyn_spawn_covered_inf = {
    params ["_pos", "_dir", ["_tree", false], ["_net", false], ["_sandBag", false], ["_bushes", false]];
    if (_tree) then {
    ...
    [_grp, _dir, _net, _sandBag, _bushes] spawn {
        params ["_grp", "_dir", "_net", "_sandBag", "_bushes"];
        private _covers = [];
...
dyn_spawn_dimounted_inf = {
    ...
    _grp = grpNull;
    _vicType = selectRandom ["cwr3_o_ural_open", "cwr3_o_ural"];
    if (_armed) then {
        _vicType = selectRandom ["cwr3_o_uaz_dshkm", "cwr3_o_uaz_ags30"];
    };
    _road = selectRandom (_pos nearRoads _area);
...
dyn_spawn_hq_garrison = {
    ...
    _grp = [_pos, east, (configFile >> "CfgGroups" >> "East" >> "CWR3_RUS" >> "Infantry" >> "cwr3_o_fire_team")] call BIS_fnc_spawnGroup;
...
dyn_spawn_aa = {
    ...
    _grp = [_iPos, east, (configFile >> "CfgGroups" >> "East" >> "CWR3_RUS" >> "Infantry" >> "cwr3_o_fire_team")] call BIS_fnc_spawnGroup;
    _grp
};

dyn_spawn_qrf = {
    params ["_trg", "_pos", "_area", "_amount"];

    waitUntil {sleep 1; triggerActivated _trg};

    private _atkPos = {
        if ((_x distance2D _pos) < _area) exitWith {getPos _x};
    } forEach (allUnits+vehicles select {side _x == west});

    private _qrf = [];
    for "_i" from 0 to (_amount - 1) do {
        _sPos = [[[_pos, 40]], ["water"]] call BIS_fnc_randomPos;
...
dyn_spawn_patrol = {
    ...
    _grp = [_startPos, east, (configFile >> "CfgGroups" >> "East" >> "CWR3_RUS" >> "Infantry" >> "cwr3_o_at_team")] call BIS_fnc_spawnGroup;
    _grp setBehaviour "SAFE";
    {
...
dyn_spawn_forest_patrol = {
    ...
    for "_i" from 0 to (_amount - 1) do {
        ...
        _grp = [_pPos, east, (configFile >> "CfgGroups" >> "East" >> "CWR3_RUS" >> "Infantry" >> "cwr3_o_fire_team")] call BIS_fnc_spawnGroup;
        _grp setBehaviour "SAFE";
...
dyn_spawn_side_town_guards = {
...

    {
        ...
        _grp = [getPos _x, east, (configFile >> "CfgGroups" >> "East" >> "CWR3_RUS" >> "Infantry" >> "cwr3_o_fire_team")] call BIS_fnc_spawnGroup;
        dyn_all_side_town_guards pushBack _grp;
        _dir = (getPos _x) getDir player;
        [_validBuildings#0, _grp, _dir] spawn dyn_garrison_building;
        _allGrps pushBack _grp;
        _vicGrp = [getPos (_validBuildings#0), 60, true, true] spawn dyn_spawn_dimounted_inf;
...
dyn_spawn_supply_convoy = {
    ...
    for "_i" from 0 to 1 step 1 do {
        ...
        _vic setDir _vDir;
        _grp = [_rearPos, east, (configFile >> "CfgGroups" >> "East" >> "CWR3_RUS" >> "Infantry" >> "cwr3_o_fire_team")] call BIS_fnc_spawnGroup;
        {
...
dyn_spawn_counter_attack = {
    params ["_trg", "_atkPos", "_defPos", "_inf", "_vics", "_breakPoint", ["_mech", false], ["_vicTypes", ["cwr3_o_bmp1", "cwr3_o_btr80", "cwr3_o_bmp2", "cwr3_o_t55"]]];
    ...
    _rearPos = [1200 * (sin _dir), 1200 * (cos _dir), 0] vectorAdd _atkPos;
